const express = require('express');
const router = express.Router();
const multer = require('multer');
const Movie = require('../models/movie');
const { getAllMovies, createMovie, addMovie} = require('../controller/movie');

router.get('/', getAllMovies);
router.post('/addmovie', createMovie);


var storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'uploads/')
    },
    filename: (req, file, cb) => {
        cb(null, `${Date.now()}_${file.originalname}`)
    },
    fileFilter: (req, file, cb) => {
        const ext = path.extname(file.originalname)
        if (ext !== '.jpg' || ext !== '.png') {
            return cb(res.status(400).end('only jpg, png are allowed'), false);
        }
        cb(null, true)
    }
})

var upload = multer({ storage: storage }).single("file")


//=================================
//             Movie
//=================================

router.post("/uploadImage", (req, res) => {

    upload(req, res, err => {
        if (err) {
            return res.json({ success: false, err })
        }
        return res.json({ success: true, image: res.req.file.path, fileName: res.req.file.filename })
    })

});


router.post("/uploadMovie", (req, res) => {

    //save all the data we got from the client into the DB 
    const movie = new Movie(req.body)

    movie.save((err) => {
        if (err) return res.status(400).json({ success: false, err })
        return res.status(200).json({ success: true })
    })

});


router.post("/getMovie", (req, res) => {

    let order = req.body.order ? req.body.order : "desc";
    let sortBy = req.body.sortBy ? req.body.sortBy : "_id";
    let limit = req.body.limit ? parseInt(req.body.limit) : 100;
    let skip = parseInt(req.body.skip);

    let findArgs = {};
    let term = req.body.searchTerm;

    for (let key in req.body.filters) {

        if (req.body.filters[key].length > 0) {
            if (key === "rate") {
                findArgs[key] = {
                    $gte: req.body.filters[key][0],
                    $lte: req.body.filters[key][1]
                }
            } else {
                findArgs[key] = req.body.filters[key];
            }
        }
    }

    console.log(findArgs)

    if (term) {
        Movie.find(findArgs)
            .find({ $text: { $search: term } })
            .sort([[sortBy, order]])
            .skip(skip)
            .limit(limit)
            .exec((err, movies) => {
                if (err) return res.status(400).json({ success: false, err })
                res.status(200).json({ success: true, movies, postSize: movies.length })
            })
    } else {
        Movie.find(findArgs)
            .populate("writer")
            .sort([[sortBy, order]])
            .skip(skip)
            .limit(limit)
            .exec((err, movies) => {
                if (err) return res.status(400).json({ success: false, err })
                res.status(200).json({ success: true, movies, postSize: movies.length })
            })
    }

});


//?id=${movieId}&type=single
//id=12121212,121212,1212121   type=array 
router.get("/movie_by_id", (req, res) => {
    let type = req.query.type
    let movieIds = req.query.id

    console.log("req.query.id", req.query.id)

    if (type === "array") {
        let ids = req.query.id.split(',');
        movieIds = [];
        movieIds = ids.map(item => {
            return item
        })
    }

    console.log("movieIds", movieIds)


    //we need to find the movie information that belong to movie Id 
    Movie.find({ '_id': { $in: movieIds } })
        .exec((err, movie) => {
            if (err) return res.status(400).send(err)
            return res.status(200).send(movie)
        })
});



module.exports = router;